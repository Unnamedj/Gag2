...
// ── Dashboard ─────────────────────────────────────────────────────────────────
app.get('/', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>GaG2 Dashboard</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css"><style>body{font-family:Inter,system-ui,sans-serif}.pet-card{transition:all .2s}.pet-card:hover{transform:translateY(-2px);box-shadow:0 8px 24px -4px rgba(168,85,247,.18)}.font-mono{font-variant-numeric:tabular-nums}</style></head><body class="bg-zinc-950 text-zinc-200 min-h-screen"><div class="max-w-[1280px] mx-auto p-6"><div class="flex justify-between items-center mb-8"><div class="flex items-center gap-3"><div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-2xl">🌱</div><div><div class="font-bold text-2xl tracking-tight">GaG2 Hopper</div><div class="text-xs text-zinc-500">Grow a Garden 2 · ${PLACE_ID}</div></div></div><div class="flex gap-2 text-sm"><div class="px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span><span id="bots-count" class="font-mono font-semibold">0</span><span class="text-zinc-500">bots</span></div><div class="px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center gap-2"><i class="fa-solid fa-server text-violet-400 text-xs"></i><span id="servers-count" class="font-mono font-semibold">0</span><span class="text-zinc-500">servers</span></div></div></div><div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8"><div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 flex flex-col gap-1"><div class="text-xs text-zinc-500 uppercase tracking-wider">Pool</div><div id="stat-total" class="text-4xl font-bold">0</div><div class="text-xs text-zinc-600">total servers</div></div><div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 flex flex-col gap-1"><div class="text-xs text-zinc-500 uppercase tracking-wider">Available</div><div id="stat-avail" class="text-4xl font-bold text-emerald-400">0</div><div class="text-xs text-zinc-600">ready to dispense</div></div><div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 flex flex-col gap-1"><div class="text-xs text-zinc-500 uppercase tracking-wider">Pets Found</div><div id="stat-pets" class="text-4xl font-bold text-violet-400">0</div><div id="stat-best" class="text-xs text-zinc-600 truncate">—</div></div><div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 flex flex-col gap-1"><div class="text-xs text-zinc-500 uppercase tracking-wider">Bots</div><div id="stat-bots" class="text-4xl font-bold">0</div><div class="text-xs text-zinc-600">active scrapers</div></div></div><div class="flex justify-between items-center mb-4 gap-4"><h2 class="font-semibold text-xl flex items-center gap-2"><span>🐾</span> Recent Pets</h2><div class="flex gap-2 flex-wrap"><input id="search-input" placeholder="Search pet name…" class="bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-2 text-sm w-52 focus:outline-none focus:border-violet-500 transition-colors"><div class="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-2xl px-3 py-2"><span class="text-xs text-zinc-500">Min $</span><input id="min-value" type="number" value="0" class="bg-transparent text-sm w-20 focus:outline-none font-mono"></div></div></div><div id="pets-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8"></div><div id="no-pets" class="text-center py-16 text-zinc-600 hidden"><div class="text-4xl mb-3">🌿</div><div class="text-sm">No pets detected yet — bots are scanning</div></div><div class="mt-4"><h2 class="font-semibold text-xl mb-4 flex items-center gap-2"><i class="fa-solid fa-server text-violet-400 text-sm"></i> Server Pool</h2><div id="pool-stats" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3"></div></div></div><script>const PLACE_ID=${PLACE_ID};function fv(v){if(!v||v<=0)return'0';if(v>=1e12)return(v/1e12).toFixed(2)+'T';if(v>=1e9)return(v/1e9).toFixed(2)+'B';if(v>=1e6)return(v/1e6).toFixed(1)+'M';if(v>=1e3)return(v/1e3).toFixed(0)+'K';return Math.floor(v)}function pe(n){n=n.toLowerCase();if(n.includes('frog'))return'🐸';if(n.includes('bunny')||n.includes('rabbit'))return'🐰';if(n.includes('owl'))return'🦉';if(n.includes('raccoon'))return'🦝';if(n.includes('gnome'))return'🧙';if(n.includes('fox'))return'🦊';if(n.includes('deer'))return'🦌';if(n.includes('squirrel'))return'🐿️';if(n.includes('snail'))return'🐌';if(n.includes('bird'))return'🐦';if(n.includes('golden'))return'✨';if(n.includes('rainbow'))return'🌈';return'🐾'}function vc(v){if(v>=1e9)return'bg-red-500/90 text-white';if(v>=1e8)return'bg-orange-500/90 text-white';if(v>=1e7)return'bg-amber-400 text-zinc-950';if(v>=1e6)return'bg-yellow-300 text-zinc-950';if(v>=1e5)return'bg-zinc-600 text-zinc-100';return'bg-zinc-800 text-zinc-400'}function ago(ms){const s=Math.floor(ms/1000);if(s<60)return s+'s ago';if(s<3600)return Math.floor(s/60)+'m ago';return Math.floor(s/3600)+'h ago'}async function loadStats(){try{const r=await fetch('/stats');const d=await r.json();document.getElementById('stat-total').innerText=d.pool.totalServers;document.getElementById('stat-avail').innerText=d.pool.availableServers;document.getElementById('stat-pets').innerText=d.pets.totalTracked;document.getElementById('stat-bots').innerText=d.bots;document.getElementById('bots-count').innerText=d.bots;document.getElementById('servers-count').innerText=d.pool.totalServers;if(d.pets.bestPetName)document.getElementById('stat-best').innerText=pe(d.pets.bestPetName)+' '+d.pets.bestPetName+' $'+fv(d.pets.bestValue);document.getElementById('pool-stats').innerHTML=['Total '+d.pool.totalServers,'Fresh '+d.pool.freshServers,'Available '+d.pool.availableServers,'Recycling '+d.pool.recyclingServers,'Dispensed '+d.pool.dispensedServers,'Removed '+d.pool.removedServers].map((l,i)=>{const cols=['text-zinc-300','text-emerald-400','text-sky-400','text-amber-400','text-violet-400','text-red-400'];return`<div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-3 text-center"><div class="${cols[i]} font-bold text-2xl">${l.split(' ')[1]}</div><div class="text-xs text-zinc-600 mt-1">${l.split(' ')[0]}</div></div>`}).join('')}catch(e){}}async function loadPets(){try{const min=document.getElementById('min-value').value||0;const q=encodeURIComponent(document.getElementById('search-input').value||'');const r=await fetch('/api/pets?minValue='+min+'&search='+q);const d=await r.json();const g=document.getElementById('pets-grid');const empty=document.getElementById('no-pets');if(!d.pets||d.pets.length===0){g.innerHTML='';empty.classList.remove('hidden');return}empty.classList.add('hidden');g.innerHTML=d.pets.map(p=>`<div class="pet-card bg-zinc-900 border border-zinc-800 rounded-3xl p-5"><div class="flex items-start justify-between gap-2 mb-3"><div class="flex items-center gap-3"><span class="text-4xl leading-none">${pe(p.pet.name)}</span><div><div class="font-semibold leading-tight">${p.pet.name}</div><div class="text-xs text-zinc-500 mt-0.5">${ago(Date.now()-p.receivedAt)}</div></div></div><span class="px-2.5 py-1 rounded-xl text-xs font-bold ${vc(p.pet.value||0)} whitespace-nowrap">$${fv(p.pet.value||0)}</span></div><div class="font-mono text-xs text-violet-400 truncate mb-1">${p.jobId}</div><div class="flex justify-between text-xs text-zinc-500 mb-3"><span>👥 ${p.players}/${p.maxPlayers||'?'}</span><span>⏱ ${p.pet.time||'?'}</span></div><div class="grid grid-cols-2 gap-2"><button onclick="copyJoin('${p.jobId}')" class="py-2 text-xs bg-violet-600 hover:bg-violet-500 rounded-xl transition-colors font-semibold">📋 Copy</button><button onclick="joinNow('${p.jobId}')" class="py-2 text-xs border border-zinc-700 hover:bg-zinc-800 rounded-xl transition-colors">Join</button></div></div>`).join('')}catch(e){}}function copyJoin(j){const cmd=\`game:GetService("TeleportService"):TeleportToPlaceInstance(\${PLACE_ID},"\${j}")\`;navigator.clipboard.writeText(cmd);const t=document.createElement('div');t.className='fixed bottom-6 left-1/2 -translate-x-1/2 bg-violet-700 px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg';t.innerText='✓ Copied!';document.body.appendChild(t);setTimeout(()=>t.remove(),1500)}function joinNow(j){prompt('Run in executor:',\`game:GetService("TeleportService"):TeleportToPlaceInstance(\${PLACE_ID},"\${j}")\`)}let debT;function setup(){const s=document.getElementById('search-input');const m=document.getElementById('min-value');const trig=()=>{clearTimeout(debT);debT=setTimeout(loadPets,280)};s.oninput=trig;m.oninput=trig}async function init(){setup();await Promise.all([loadStats(),loadPets()]);setInterval(()=>Promise.all([loadStats(),loadPets()]),8000)}window.onload=init</script></body></html>`);
});
